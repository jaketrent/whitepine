...
$(document).ready(function () {
  ...
  $.ajax({
    url: 'http://picasaweb.google.com/data/feed/api/user/116342059677336243524?alt=json',
    dataType: 'json',
    success: function(body) {
      var $frag = $('<albums/>');
      $(body.feed.entry).each(function (indx, album) {
        $frag.append('<li><a href="#" data-albumid="' + album.gphoto$id.$t + '" class="album"><img src="' + album.media$group.media$thumbnail[0].url + '" /></a></li>');
      });
      $('.albums').html($frag.html());
      $(".album").click(function () {
        var albumId = $(this).attr('data-albumId');
        $.ajax({
          url: 'http://picasaweb.google.com/data/feed/api/user/116342059677336243524/albumid/' + albumId + '?alt=json&thumbsize=1024',
          dataType: 'json',
          success: function(body) {
            $('#disp-photo').attr('src', body.feed.entry[0].media$group.media$thumbnail[0].url);
            $('.albums').animate({
              left: 1024,
              opacity: 0
            }, {
